...
public  class HttpURLConnection {
    ...
    public synchronized  void sendPostAvailableAccountBalances() throws Exception {
        ...
        Gson gson = new Gson();
        try {
            mapBalance = gson.fromJson(resp, new TypeToken<Map<String, HashMap<String, String>>>() {
            }.getType());
        } catch (Exception e) {
    ...
    public synchronized void sendPostReturnOpenOrders(String currencyPair) throws Exception {
        ...
        String queryArgs = "command=returnOpenOrders&currencyPair="+currencyPair+"&nonce=" + nonce;
        ...
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        params.add(new BasicNameValuePair("command", "returnOpenOrders"));
        params.add(new BasicNameValuePair("currencyPair", currencyPair));
        params.add(new BasicNameValuePair("nonce", nonce));
        post.setEntity(new UrlEncodedFormEntity(params));
